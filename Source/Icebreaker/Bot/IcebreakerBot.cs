...
namespace Icebreaker.Bot
{
    ...
    public class IcebreakerBot : TeamsActivityHandler
    {
        ...
        protected override async Task OnMembersAddedAsync(IList<ChannelAccount> membersAdded, ITurnContext<IConversationUpdateActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            using (this.telemetryClient.StartOperation<RequestTelemetry>(Contants.OnMembersAddedAsyncOperation, operationCorrelationId.ToString()))
            {
                try
                {
                    if (membersAdded?.Count() > 0)
                    {
                        ...
                        foreach (var member in membersAdded)
                        {
                            ...
                            {
                                //https://stackoverflow.com/questions/52688906/applicationinsights-operationid-is-empty/52717627#52717627
        ...
        private async Task HandleMessageActivityAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            ...
            using (this.telemetryClient.StartOperation<RequestTelemetry>(Contants.OnMessageActivityAsyncOperation, operationCorrelationId.ToString()))
            {
                try
                {
